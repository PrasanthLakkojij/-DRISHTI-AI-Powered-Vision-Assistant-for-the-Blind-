...
def get_lang_changed_msg(lang_key: str) -> str:
    return {"telugu":"భాష తెలుగుకు మార్చబడింది","hindi":"भाषा हिंदी में बदल दी गई है",
            ...
            "gujarati":"ભાષા ગુજरাதীમાં બدলвामां આवी","urdu":"زبان اردو میں تبدیل کر دی گئی",
...
def _lc_ask_prompt(lang_key: str, lang_list: str) -> str:
    ...
    return {
        ...
        "kannada":f"ಯಾವ ಭಾಷೆಗೆ ಬದಲಾಯಿಸಬೇಕು? ಲಭ್ಯವಿರುವವು: {lang_list}",
...
def _lc_not_heard_msg(lang_key: str) -> str:
    ...
    return {
        ...
        "malayalam":"കേട്ടില്ല. പറയുക: ഹിന്ദി, തമിഴ്, കന്നഡ, ഇംഗ്ലീഷ്",
...
def _lc_not_understood_msg(lang_key: str, supported: str) -> str:
    ...
    return {
        ...
        "kannada":f"ಅರ್ಥವಾಗಲಿಲ್ಲ. ಲಭ್ಯವಿರುವವು: {supported}. ಮತ್ತೆ ಹೇಳಿ",
...
def _lc_give_up_msg(lang_key: str) -> str:
    ...
    return {
        ...
        "kannada":"ಭಾಷೆ ಗುರುತಿಸಲಾಗಲಿಲ್ಲ. ಮತ್ತೆ activate ಎಂದು ಹೇಳಿ ಪ್ರಯತ್ನಿಸಿ",
...
def _groq_resize_frame_for_upload(frame):
    height, width = frame.shape[:2]
    if width <= GROQ_FRAME_MAX_WIDTH:
        return frame
    scale = GROQ_FRAME_MAX_WIDTH / width
    new_height = int(height * scale)
    return cv2.resize(frame, (GROQ_FRAME_MAX_WIDTH, new_height), interpolation=cv2.INTER_AREA)

def _groq_frame_to_base64_jpeg(frame) -> str:
    frame = _groq_resize_frame_for_upload(frame)
    success, encoded = cv2.imencode(
        ".jpg", frame, [int(cv2.IMWRITE_JPEG_QUALITY), GROQ_JPEG_QUALITY]
    )
    if not success:
...
def _voice_command_thread() -> None:
    ...
    with mic as src:
        ...
        recognizer.energy_threshold=max(150,min(400,ambient*1.5))
...
PROMPT_OCR_READ = (
    "Read any text visible in this image and transcribe it exactly as written, "
    "in its original language and script. Output ONLY the text itself, nothing else — "
    "no descriptions, no preamble, no translation. If there is no readable text, "
...
def _read_text_with_groq(frame_bgr) -> str:
    """Groq-based replacement for EasyOCR. Reuses the same frame resize/
    JPEG-encode helpers built for the describe feature."""
    if _groq_client is None:
        raise RuntimeError("Groq API key not set")
    base64_image = _groq_frame_to_base64_jpeg(frame_bgr)
    completion = _groq_client.chat.completions.create(
        model=GROQ_SCENE_MODEL,
        messages=[
            {
