...
import cv2, threading, tempfile, pickle, queue, audioop
from pathlib import Path
from PIL import Image
...
# Confirmed via testing: this earphone reports clean, distinct events —
# double-clicking the main button fires as 'next track', and Volume
# Up/Down are separate real buttons. No sustained 'hold' signal exists
# (down/up land at the same instant even on long presses), so the
# trigger uses these discrete click patterns instead of a hold duration.
try:
...
LANG_SPOKEN_FORMS = {
    ...
    "tamil"    :["tamil","Tamil","తమిళం","తమిళ్","तमिल","தமிழ்","ತಮಿಳు","തമിഴ്"],
    ...
    "gujarati" :["gujarati","Gujarati","గుజరాతీ","गुजराती","குஜராத்தி","ಗುಜರಾತಿ","ഗുജറാത്തി","ગુજરાતી"],
...
def _lc_ask_prompt(lang_key: str, lang_list: str) -> str:
    ...
    return {
        ...
        "kannada":f"ಯಾವ ಭಾಷೆಗೆ ಬದಲಾಯಿಸಬೇಕು? ಲಭ್ಯವಿರುವவு: {lang_list}",
        "malayalam":f"ഏത് ഭാഷയിലേക്ക് മാറ്റണം? ലഭ്യമായവ: {lang_list}",
...
def _lc_not_heard_msg(lang_key: str) -> str:
    ...
    return {
        ...
        "kannada":"ಕೇಳಿಸಲಿಲ್ಲ. ಹೇಳಿ: ಹಿಂದಿ, ತಮಿಳು, ಕನ್ನಡ, ಇಂಗ್ಲಿಷ್",
        "malayalam":"കേട്ടില്ല. പറയുക: ഹിന്ദি, തമിഴ്, കന്നഡ, ഇംഗ്ലീഷ്",
...
def _lc_not_understood_msg(lang_key: str, supported: str) -> str:
    ...
    return {
        ...
        "tamil":f"புரியவில்லை. கிடைக்கும்: {supported}. மீண்டும் சொல்லுங்கள்",
        "kannada":f"ಅರ್ಥವಾಗಲಿಲ್ಲ. ಲಭ್ಯವಿರುವவு: {supported}. ಮತ್ತೆ ಹೇಳಿ",
        "malayalam":f"മനസ്സിലായില്ല. ലഭ്യമായവ: {supported}. വീണ്ടും പറയുക",
...
def _lc_give_up_msg(lang_key: str) -> str:
    ...
    return {
        ...
        "tamil":"மொழி அடையாளம் காணப்படவில்லை. மீண்டும் activate சொல்லி முயற்சிக்கவும்",
        "kannada":"ಭಾಷೆ ಗುರುತಿಸಲಾಗಲಿಲ್ಲ. ಮತ್ತೆ activate ಎಂದு ಹೇಳಿ ಪ್ರಯತ್ನಿಸಿ",
        "malayalam":"ഭാഷ തിരിച്ചറിഞ്ഞില്ല. വീണ്ടും activate എന്ന് പറഞ്ഞ് ശ്രമിക്കുക",
...
# ── EARPHONE BUTTON ACTIVATION (confirmed reliable by direct testing) ──
# Two independent physical triggers, both fire as clean discrete events
# on this earphone:
#   1. Double-click the main button  -> OS reports it as 'next track'
#   2. Triple-click either volume button within HOTKEY_TRIPLE_WINDOW
# No hold-duration signal exists on this hardware (down/up are
# simultaneous even on long presses), so both triggers use click
# patterns rather than hold time.
_manual_activate_pending = threading.Event()
HOTKEY_TRIPLE_WINDOW = 2.0   # seconds — 3 volume-button presses within this window triggers activation
_hotkey_vol_press_times = []
_hotkey_vol_lock = threading.Lock()

# Fallback for earphones that only ever send 'play/pause media' (no
# separate volume/next-track signals): two plain presses of that one
# button, a few seconds apart, also triggers activation. Window is
# generous (not a fast double-click) since a relaxed press-press-pause
# is easier to do reliably than a fast click.
HOTKEY_PLAYPAUSE_WINDOW = 4.0
_hotkey_pp_press_times = []
_hotkey_pp_lock = threading.Lock()

def _on_next_track(event) -> None:
    print("[Hotkey] Double-click detected ('next track') — manual activation triggered")
    _manual_activate_pending.set()

def _on_volume_press(event) -> None:
    now = time.time()
    with _hotkey_vol_lock:
        _hotkey_vol_press_times.append(now)
        _hotkey_vol_press_times[:] = [t for t in _hotkey_vol_press_times if now - t <= HOTKEY_TRIPLE_WINDOW]
        if len(_hotkey_vol_press_times) >= 3:
            print("[Hotkey] Triple volume-button press detected — manual activation triggered")
            _manual_activate_pending.set()
            _hotkey_vol_press_times.clear()

def _on_playpause_press(event) -> None:
    now = time.time()
    with _hotkey_pp_lock:
        _hotkey_pp_press_times.append(now)
        _hotkey_pp_press_times[:] = [t for t in _hotkey_pp_press_times if now - t <= HOTKEY_PLAYPAUSE_WINDOW]
        if len(_hotkey_pp_press_times) >= 2:
            print("[Hotkey] Double play/pause press detected — manual activation triggered")
            _manual_activate_pending.set()
            _hotkey_pp_press_times.clear()

def _hotkey_key_event_callback(event) -> None:
    """Generic key-event handler (not on_press_key) — this earphone's
    buttons use unusual negative scan codes that on_press_key's internal
    name-lookup table doesn't recognize, so we hook every key event
    directly and check event.name ourselves, exactly like the working
    diagnostic script did."""
    if getattr(event, "event_type", None) != "down":
        return
    name = (event.name or "").lower()
    if name == "next track":
        _on_next_track(event)
    elif name in ("volume up", "volume down"):
        _on_volume_press(event)
    elif name == "play/pause media":
        _on_playpause_press(event)

def _hotkey_watcher_thread() -> None:
    """Hooks ALL key events (generic hook, not on_press_key — see
    _hotkey_key_event_callback for why) and sets _manual_activate_pending
    on any of: double-click ('next track'), triple volume-button press,
    or double play/pause press — a fallback activation path that doesn't
    depend on the wake word being heard at all. Requires the 'keyboard'
    library; no-ops silently if missing or if hooking fails."""
    if not KEYBOARD_OK:
        return
    try:
        _kb.hook(_hotkey_key_event_callback)
        print("[Hotkey] Watching earphone buttons (generic hook): double-click (next track) OR "
              "triple volume-press OR double play/pause-press...")
    except Exception as e:
...
def _activate_and_listen_for_command(recognizer, src, wake_text: str) -> None:
    ...
    and when the earphone volume-button fallback fires (wake_text="",
...
def _voice_command_thread() -> None:
    ...
    with mic as src:
        ...
        while not _vc_stop.is_set():
            if pygame.mixer.music.get_busy():
                time.sleep(0.1); continue
            # ── EARPHONE VOLUME-BUTTON FALLBACK ─────────────────────
            # A short (1s) timeout instead of blocking forever lets this
            # loop wake up regularly even in total silence, so it can
            # notice the Volume Up+Down 3-second-hold signal from
            # _hotkey_watcher_thread and jump straight into command mode
            # without "listen" needing to be heard/understood at all.
            try:
...
def orb_direction(frame_bgr,dest_name):
    result={"direction":"forward","offset_x":0,"offset_y":0,"arrow_dst":None,"n_matches":0,"confidence":0.0,"scale":0.0}
            ...
            if now-last_speak_time>SPEAK_COOLDOWN:
                ...
                if arrived:
                    ...
                    if lang=="english":
                        ...
                        speak({"left":"ಎಡಕ್ಕೆ ತಿರುಗಿ","right":"ಬಲಕ್ಕೆ ತಿరுగಿ","forward":"ಮುಂదே ಹೋಗಿ"}.get(direction,"ಮುಂదే ಹೋಗಿ"))
                    elif lang=="malayalam":
                        speak({"left":"ഇടത്തോട്ട് తిరియుక","right":"వలత్తోట్ట్ తిరియుక","forward":"మున్నోట్ట్ పోకుక"}.get(direction,"మున్నోట్ట్ పోకుక"))
                    else:
                ...
                if frame_count%FRAME_SKIP==0:
                    ...
                    if yolo_cache and now-last_yolo_speak>YOLO_COOLDOWN:
                        ...
                        if lang=="english":
                            ...
                            speak(f"{sv0} {tel} {d0} ಮೀಟర్ ದూరదల్లిదే ఎచ్చరిక")
                        elif lang=="malayalam":
                ...
                for item in sorted_faces:
                    ...
                    if now-last_t>=FACE_COOLDOWN:
                        ...
                        if lang=="english":
                            ...
                            sv={"Left":"இடது பக்கం","Right":"வலது பக்కం","Center":"முன்னால்"}.get(side,side)
