...
DRISHTI is an AI-powered voice assistant that helps blind and low-vision
individuals navigate the world independently. It combines real-time object
detection, face recognition, voice authentication, AI scene description,
text reading, place navigation, a general AI assistant, and emergency SOS —
all activated by saying **"Hey Jarvis"** and controlled entirely by voice.
...
```
⚠️ **Note:** `face_recognition` requires `cmake` and `dlib`. If installation
fails, run these first:
```
pip install cmake
pip install dlib
...
DRISHTI's AI features (Describe, AI Mode, Text Reading) and emergency
features (SOS, SMS) need API keys. Create a file named `.env` in the project
folder:
```
...
On the very first run, DRISHTI introduces itself and asks you to enroll your
voice (3 short voice samples) before it can be used.

## 🎮 Voice Commands

All commands below are spoken **after saying "listen"** to wake DRISHTI into
command mode.

| Say This | Action |
|---|---|
| "Hey Jarvis" | Wakes DRISHTI from sleep |
| "listen" | Opens the voice command menu |
| "menu" | Lists all available commands aloud |
| "save person" | Learns and saves a new face |
| "save place" | Learns and saves a new location |
| "where is [place]" / "navigate" | Starts guided navigation to a saved place |
| "stop navigation" | Exits navigation mode |
| "read text" | Reads any visible text aloud |
| "describe" | Describes the full surrounding scene aloud |
| "ai mode" | Starts a general AI Q&A conversation |
| "change language" | Switches the spoken language |
| "send message" | Dictates and sends an SMS |
| "s.o.s." | Places an emergency call via Twilio |
| "cancel" | Stops the current mode (describe, AI mode, navigation, reading) |
...
> **Note:** Model weight files (`yolov8n.pt`) and SpeechBrain's cached models
> auto-download on first run. Personal/biometric files generated on your own
> device — `owner_voice.npy`, `owner_recordings/`, `saved_faces/`,
> `saved_places/`, `face_db.pkl` — are intentionally excluded from this
> repository and are not shared between devices.
